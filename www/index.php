...
if ($active_page_id > 0) {
  ...
  $initial_widgets = array_map(function ($r) {
    $pos = json_decode($r['position_data'], true) ?: [];
    ...
    return [
      ...
      'position_w' => (int) ($pos['w'] ?? 1),
      'position_h' => (int) ($pos['h'] ?? 1),
      'position_h_px' => (int) ($pos['h_px'] ?? 0),
    ];
  }, $stmt->fetchAll());
}
...
<div class="app">
  ...
  <nav class="navbar">
      ...
      <div class="dashboard-menu" id="dashboard-menu">
        <div class="dashboard-menu__list" id="dashboard-menu-list">
          <?php foreach ($all_dashboards as $d): ?>
            ...
            <div
              class="dashboard-menu__item <?= $is_active ? 'dashboard-menu__item--active' : '' ?> <?= $is_locked_dash ? 'dashboard-menu__item--locked' : '' ?>"
    ...
    try {
      ...
      $_idx_sub = ['slug' => 'free', 'plan_name' => 'Free'];
    }
    ?>
    <a href="/billing.php" class="nav-billing nav-billing--<?= $_idx_sub['slug'] ?>" title="Подписка">
      <?= icon('zap', 'nav-billing__icon', 15) ?>
      <span
        class="nav-billing__plan nav-billing__plan--<?= $_idx_sub['slug'] ?>"><?= htmlspecialchars($_idx_sub['plan_name']) ?></span>
    </a>
