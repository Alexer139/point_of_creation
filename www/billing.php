    ...
    <div class="downgrade-banner">
      ...
      <div class="downgrade-banner__body">
        ...
        <div class="downgrade-banner__sub">
          ...
          у вас <?= $usage_dashboards ?>. Выберите какие оставить активными — остальные будут заморожены (данные
          сохранятся).
        </div>
      </div>
          ...
          <div
            ...
            <?php elseif ($price > 0 && !$can_afford): ?>
              <button class="btn btn--ghost btn--full"
                onclick="document.getElementById('topup-amount').value=<?= ceil($price - (float) $wallet['balance']) ?>;document.getElementById('topup-amount').focus()"
                title="Недостаточно средств">
                <?= icon('plus', '', 13) ?> Пополнить на
                <?= number_format(ceil($price - (float) $wallet['balance']), 0, '.', ' ') ?> ₽
              </button>
            <?php elseif ($is_downgrade): ?>
              <!-- Даунгрейд: два варианта -->
              <button class="btn btn--ghost btn--full"
                onclick="activatePlan('<?= $plan['slug'] ?>', '<?= addslashes($plan['name']) ?>', 0, true, false)"
                title="Переход после окончания текущего периода">
                <?= icon('clock', '', 13) ?> Со следующего периода
              </button>
              <button class="btn btn--danger btn--full" style="margin-top:.4rem"
                onclick="activatePlan('<?= $plan['slug'] ?>', '<?= addslashes($plan['name']) ?>', 0, false, true)"
                title="Немедленный переход — текущая подписка отменяется">
                <?= icon('arrow-down', '', 13) ?> Перейти сейчас
              </button>
            <?php else: ?>
              <button class="btn btn--warm btn--full"
                onclick="activatePlan('<?= $plan['slug'] ?>', '<?= addslashes($plan['name']) ?>', <?= $final_price ?>, false, false)">
...
<style>
  ...
  .txn-item--refund .txn-item__icon {
    color: #6366f1;
  }
  ...
  .downgrade-banner__icon {
    color: var(--amber);
    flex-shrink: 0;
  }

  .downgrade-banner__body {
    flex: 1;
  }

  .downgrade-banner__title {
    font-weight: 700;
    color: var(--text-1);
    margin-bottom: .2rem;
  }

  .downgrade-banner__sub {
    font-size: .83rem;
    color: var(--text-2);
  }
  ...
  .downgrade-item input[type=checkbox] {
    width: 16px;
    height: 16px;
    flex-shrink: 0;
    cursor: pointer;
...
<script>
  ...
  async function activatePlan(slug, name, price) {
    ...
    try {
      const r = await api('activate_plan', { slug, force: force ? 1 : 0 });
      if (r.ok) {
