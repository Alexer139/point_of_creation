...
// ИСПРАВЛЕНО: виджеты больше не привязаны к user_id напрямую,
// считаем через pages → dashboards → owner_id
$users = $db->query("
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && verify_csrf($_POST['csrf'] ?? '')) {
    ...
    if ($tid > 0 && $tid !== (int)current_user()['id']) {
        // Каскадное удаление сработает автоматически (FK ON DELETE CASCADE)
        $db->prepare('DELETE FROM users WHERE id = ?')->execute([$tid]);
        header('Location: /admin.php?msg=deleted'); exit;
    }
}

$flash_map = ['deleted' => '✓ Пользователь и все его данные удалены.'];
...
<div class="admin-content">
  ...
  <div class="admin-table-card">
      ...
      <tbody>
        <?php foreach ($users as $u):
          ...
          <tr>
            ...
            <td>
              <div style="display:flex;align-items:center;gap:.625rem">
                ...
                <div>
                  ...
                  <?php if ($isSelf): ?>
                    <div style="font-size:.7rem;color:var(--amber);font-weight:600">это вы</div>
                  <?php endif; ?>
                </div>
              </div>
            </td>
            ...
            <td>
              <?php if (!$isSelf): ?>
                <form method="POST" onsubmit="return confirm('Удалить «<?= htmlspecialchars($u['username'],ENT_QUOTES) ?>» и все его данные?')">
    ...
    <?php if (!$users): ?>
      <div style="padding:3rem;text-align:center;color:var(--text3)">Нет пользователей</div>
    <?php endif; ?>
  </div>

  <div class="admin-footer-note">
    💡 При удалении пользователя все его дашборды, страницы и виджеты удаляются автоматически (CASCADE). Вы не можете удалить себя.
  </div>
</div>
